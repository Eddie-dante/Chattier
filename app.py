...
LUXURY_REACTIONS = {
    "crown": {"emoji": "👑", "label": "Top Tier", "color": "#FFD700", "glow": "#FFD700"},
    "diamond": {"emoji": "💎", "label": "Brilliant", "color": "#B9F2FF", "glow": "#00FFFF"},
    "cheers": {"emoji": "🥂", "label": "Cheers", "color": "#FFE4B5", "glow": "#FFD700"},
    "tophat": {"emoji": "🎩", "label": "Class", "color": "#1a1a1a", "glow": "#C0C0C0"},
    "sparkle": {"emoji": "✨", "label": "Premium", "color": "#FFF8DC", "glow": "#FFD700"},
    "fleur": {"emoji": "⚜️", "label": "Royal", "color": "#FFD700", "glow": "#FFA500"},
    "fire": {"emoji": "🔥", "label": "Fire", "color": "#FF4500", "glow": "#FF6347"},
    "star": {"emoji": "🌟", "label": "Star", "color": "#FFD700", "glow": "#FFFF00"},
    "love": {"emoji": "💖", "label": "Love", "color": "#FF69B4", "glow": "#FF1493"},
    "gold": {"emoji": "🏆", "label": "Champion", "color": "#FFD700", "glow": "#DAA520"},
}

# ========== AVATAR COLORS (Pure Code) ==========
AVATAR_COLORS = [
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100">
  ...
  <circle cx="50" cy="50" r="48" fill="url(#maleGrad)" stroke="#FFD700" stroke-width="2" filter="url(#maleGlow)"/>
  <circle cx="50" cy="38" r="14" fill="#f1f5f9"/>
  <ellipse cx="50" cy="72" rx="20" ry="16" fill="#f1f5f9"/>
</svg>
"""
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100">
  ...
  <circle cx="50" cy="50" r="48" fill="url(#femaleGrad)" stroke="#FFD700" stroke-width="2" filter="url(#femaleGlow)"/>
  <circle cx="50" cy="38" r="14" fill="#fce4ec"/>
  <ellipse cx="50" cy="72" rx="18" ry="14" fill="#fce4ec"/>
  <path d="M35 75 Q50 90 65 75" stroke="#fce4ec" stroke-width="3" fill="none"/>
</svg>
"""
...
# ========== 30 WALLPAPERS ==========
WALLPAPERS = {
    "wp_gradient": {"name": "Luxury Gradient", "icon": "🌈", "url": None, "gradient": "linear-gradient(135deg, #1a0033 0%, #4a0066 25%, #800080 50%, #4a0080 75%, #1a0033 100%)"},
...
def get_gender_avatar(username: str, size: int = 36, is_female: bool = False) -> str:
    """Generate pure-code SVG avatar based on gender"""
    svg = FEMALE_AVATAR_SVG if is_female else MALE_AVATAR_SVG
    b64 = base64.b64encode(svg.encode()).decode()
    return f'<img src="data:image/svg+xml;base64,{b64}" class="avatar" style="width:{size}px;height:{size}px;" alt="{username}">'
...
class DataManager:
    ...
    def _default_profile(username: str) -> Dict:
        return {"display_name": username, "bio": "", "avatar": None, "website": "", "location": "", "is_verified": False, "last_seen": "", "followers": [], "following": [], "blocked": [], "post_count": 0, "theme": "midnight", "wallpaper": "wp_gradient", "gender": "male", "created_at": datetime.now().isoformat()}
    ...
    def add_reaction(post_id: str, reaction_key: str):
        ...
        for post in posts:
            if post["id"] == post_id:
                ...
                if u in post["reactions"][reaction_key]:
                    post["reactions"][reaction_key].remove(u)
                else:
                    post["reactions"][reaction_key].append(u)
                if not post["reactions"][reaction_key]: del post["reactions"][reaction_key]
                if not post["reactions"]: del post["reactions"]
                DataManager.save_feed_posts(posts)
                st.session_state.feed_posts = posts
                return
    ...
    def get_chat_list() -> List[Dict]:
        ...
        for cid, cd in dms.items():
            if u in cd["participants"]:
                ...
                chats.append({"with_user": other, "last_message": last["text"][:40] if last and last.get("text") else "Media", "last_time": last["timestamp"] if last else cd["created_at"], "unread": unread, "is_online": other in online})
...
class FollowHandler:
    ...
    def follow(target: str) -> Tuple[bool, str]:
        ...
        profiles = DataManager.get_profiles()
        up = DataManager.get_profile(st.session_state.user); tp = DataManager.get_profile(target)
        for p in [up, tp]:
...
def inject_styles():
    ...
    * {{ font-family: 'Inter', sans-serif; }}
    ...
    .stDeployButton, [data-testid="stDecoration"], [data-testid="stStatusWidget"], [data-testid="stHeader"] {{ display: none !important; }}
    ...
    .card-header {{ display: flex !important; align-items: center !important; padding: 10px 12px !important; gap: 10px !important; }}
    .avatar {{ width: 36px !important; height: 36px !important; border-radius: 50% !important; object-fit: cover !important; border: 2px solid #FFD700 !important; flex-shrink: 0 !important; box-shadow: 0 0 10px rgba(255,215,0,0.3) !important; }}
    .avatar-placeholder {{
        width: 36px !important; height: 36px !important; border-radius: 50% !important;
        display: flex !important; align-items: center !important; justify-content: center !important;
        font-weight: 700 !important; color: white !important; font-size: 0.85rem !important;
        border: 2px solid #FFD700 !important; flex-shrink: 0 !important;
        box-shadow: 0 0 10px rgba(255,215,0,0.3) !important;
    }}
    .username-text {{ color: {theme['text']} !important; font-weight: 600 !important; font-size: 0.85rem !important; }}
    .timestamp {{ color: {theme['secondary']} !important; font-size: 0.65rem !important; }}
    .post-text {{ color: #e2e8f0 !important; font-size: 0.9rem !important; line-height: 1.6 !important; padding: 0 12px 8px 12px !important; word-wrap: break-word !important; }}
    .post-media {{ width: 100% !important; max-height: 400px !important; object-fit: cover !important; border-radius: 8px !important; }}
    .post-actions {{ display: flex !important; align-items: center !important; padding: 8px 12px !important; gap: 6px !important; border-top: 1px solid rgba(255,215,0,0.1) !important; flex-wrap: wrap !important; }}
    ...
    .luxury-reaction-btn:hover {{
        transform: scale(1.15) !important;
        background: rgba(255,215,0,0.15) !important;
        border-color: rgba(255,215,0,0.4) !important;
        box-shadow: 0 0 20px rgba(255,215,0,0.5), 0 0 40px rgba(255,215,0,0.2), 0 0 60px rgba(255,215,0,0.1) !important;
        z-index: 10 !important;
    }}

    .luxury-reaction-btn:active {{
    ...
    /* Chat bubbles */
    .chat-bubble {{ max-width: 80% !important; padding: 10px 14px !important; border-radius: 16px !important; font-size: 0.85rem !important; line-height: 1.4 !important; margin: 3px 0 !important; word-wrap: break-word !important; }}
    .chat-bubble.sent {{ background: linear-gradient(135deg, #667eea, #764ba2) !important; color: white !important; align-self: flex-end !important; border-bottom-right-radius: 4px !important; box-shadow: 0 2px 10px rgba(102,126,234,0.3) !important; }}
    .chat-bubble.received {{ background: rgba(255,255,255,0.08) !important; color: #e2e8f0 !important; align-self: flex-start !important; border-bottom-left-radius: 4px !important; }}

    /* Stories */
    .stories-row {{ display: flex !important; gap: 14px !important; padding: 10px 0 !important; overflow-x: auto !important; margin-bottom: 10px !important; }}
    .stories-row::-webkit-scrollbar {{ height: 0 !important; }}
    .story-item {{ display: flex !important; flex-direction: column !important; align-items: center !important; gap: 4px !important; min-width: 64px !important; cursor: pointer !important; }}
    .story-ring {{
        width: 60px !important; height: 60px !important; border-radius: 50% !important;
        padding: 3px !important;
        background: linear-gradient(45deg, #FFD700, #FFA500, #FFD700, #FFA500) !important;
    ...
    .story-ring-inner-placeholder {{
        ...
        display: flex !important; align-items: center !important; justify-content: center !important;
        font-weight: 700 !important; color: white !important; font-size: 1.1rem !important;
        border: 2px solid {theme['bg']} !important;
    }}
    .story-name {{ color: {theme['secondary']} !important; font-size: 0.6rem !important; max-width: 60px !important; overflow: hidden !important; text-overflow: ellipsis !important; white-space: nowrap !important; }}

    .user-row {{ display: flex !important; align-items: center !important; gap: 10px !important; padding: 8px 10px !important; border-radius: 10px !important; cursor: pointer !important; }}
    .user-row:hover {{ background: rgba(255,215,0,0.05) !important; }}
    .online-dot {{ width: 8px !important; height: 8px !important; border-radius: 50% !important; background: #10b981 !important; box-shadow: 0 0 8px rgba(16,185,129,0.6) !important; flex-shrink: 0 !important; }}
    .unread-count {{ background: #FFD700 !important; color: #1a0033 !important; border-radius: 10px !important; padding: 2px 8px !important; font-size: 0.65rem !important; font-weight: 600 !important; }}

    .modal-overlay {{ position: fixed !important; top: 0 !important; left: 0 !important; right: 0 !important; bottom: 0 !important; background: rgba(0,0,0,0.85) !important; backdrop-filter: blur(8px) !important; display: flex !important; align-items: center !important; justify-content: center !important; z-index: 2000 !important; }}
    .modal-box {{ background: {theme['bg']}fa !important; border: 2px solid rgba(255,215,0,0.2) !important; border-radius: 20px !important; width: 92% !important; max-width: 500px !important; max-height: 80vh !important; overflow-y: auto !important; padding: 20px !important; }}

    .theme-grid {{ display: grid !important; grid-template-columns: repeat(3, 1fr) !important; gap: 8px !important; padding: 8px 0 !important; }}
    .wallpaper-grid {{ display: grid !important; grid-template-columns: repeat(4, 1fr) !important; gap: 6px !important; padding: 8px 0 !important; }}
    .theme-card {{ border-radius: 12px !important; padding: 16px 6px !important; text-align: center !important; cursor: pointer !important; border: 2px solid transparent !important; transition: all 0.3s !important; }}
    .theme-card:hover {{ transform: scale(1.05) !important; box-shadow: 0 0 20px rgba(255,215,0,0.3) !important; }}
    .theme-card.selected {{ border-color: #FFD700 !important; box-shadow: 0 0 25px rgba(255,215,0,0.5) !important; }}
    .wallpaper-card {{ border-radius: 10px !important; height: 60px !important; cursor: pointer !important; border: 2px solid transparent !important; background-size: cover !important; background-position: center !important; transition: all 0.3s !important; }}
    .wallpaper-card:hover {{ transform: scale(1.08) !important; box-shadow: 0 0 20px rgba(255,215,0,0.4) !important; }}
    .wallpaper-card.selected {{ border-color: #FFD700 !important; box-shadow: 0 0 25px rgba(255,215,0,0.6) !important; }}
    ...
    .stTabs [data-baseweb="tab"] {{ color: {theme['secondary']} !important; border-radius: 8px !important; padding: 6px 14px !important; font-size: 0.8rem !important; }}
    .stTabs [aria-selected="true"] {{ color: #FFD700 !important; background: rgba(255,215,0,0.1) !important; }}

    .stExpander {{ background: {theme['card']} !important; border: 1px solid rgba(255,255,255,0.08) !important; border-radius: 12px !important; }}
    .streamlit-expanderHeader {{ color: {theme['text']} !important; font-size: 0.9rem !important; }}
    ...
    @media (max-width: 480px) {{
        .card {{ border-radius: 12px !important; margin-bottom: 8px !important; }}
        .bottom-nav {{ height: 56px !important; }}
        .main-content {{ bottom: 56px !important; }}
        .stories-row {{ gap: 10px !important; }}
        .story-ring {{ width: 52px !important; height: 52px !important; }}
    }}
    </style>
...
def render_avatar(username: str, size: int = 36) -> str:
    ...
    # Use pure-code SVG avatar
    return get_gender_avatar(username, size, is_female)

def render_story_ring(username: str, size: int = 60, has_new: bool = False) -> str:
...
def render_header():
    ...
    st.markdown(f'<div class="app-header"><div class="app-logo">👑 SocialHub</div><div style="display:flex;align-items:center;gap:12px;color:{theme["text"]};"><span style="position:relative;">🔔{badge_html}</span>{render_avatar(user, 30)}</div></div>', unsafe_allow_html=True)
...
def render_stories_bar():
    ...
    html += f'<div class="story-item">{render_story_ring(user, 60, not has_own)}<div class="story-name">You</div></div>'
    for u, ss in active.items():
        if u != user:
            has_new = any(st.session_state.user not in s.get("views", []) for s in ss)
            html += f'<div class="story-item">{render_story_ring(u, 60, has_new)}<div class="story-name">@{u[:9]}</div></div>'
    if len(active) <= 1:
        html += '<div style="color:#94a3b8;display:flex;align-items:center;font-size:0.75rem;padding-left:8px;">No stories yet</div>'
    html += '</div>'
    st.markdown(html, unsafe_allow_html=True)

def render_luxury_reaction_bar(post_id: str, reactions: Dict):
    """Render the VIP luxury reaction bar"""
    st.markdown('<div class="luxury-reaction-bar">', unsafe_allow_html=True)
    
    cols = st.columns(len(LUXURY_REACTIONS) + 2)
...
def render_post_card(post: Dict):
    ...
    # Luxury reaction bar
    render_luxury_reaction_bar(pid, reactions)

    # Comment and delete actions
    st.markdown('<div style="display:flex;align-items:center;padding:4px 12px 8px 12px;gap:8px;">', unsafe_allow_html=True)
...
def render_poll_card(post: Dict):
    ...
    for opt, voters in options.items():
        ...
        border = f"border:1px solid #FFD700;" if voted else ""
        st.markdown(f'<div style="background:rgba(255,255,255,0.04);border-radius:8px;padding:6px 8px;margin:3px 0;cursor:pointer;{border}"><div style="display:flex;justify-content:space-between;color:#e2e8f0;font-size:0.82rem;"><span>{"✓ " if voted else ""}{html.escape(opt)}</span><span>{pct:.0f}%</span></div><div style="height:3px;background:rgba(255,255,255,0.06);border-radius:2px;margin-top:4px;"><div style="width:{pct}%;height:100%;background:linear-gradient(90deg,#FFD700,#FFA500);border-radius:2px;"></div></div></div>', unsafe_allow_html=True)
        if st.button(f"Vote {html.escape(opt[:15])}", key=f"pv_{pid}_{opt[:10]}"):
            PostHandler.vote_poll(pid, opt)
            st.rerun()

    st.markdown(f'<div style="color:#94a3b8;font-size:0.65rem;margin-top:4px;">{total} votes</div></div></div>', unsafe_allow_html=True)

def render_comments(post_id: str):
    comments = CommentHandler.get(post_id)
    st.markdown('<div style="padding:4px 12px;border-top:1px solid rgba(255,215,0,0.1);">', unsafe_allow_html=True)
...
def render_chat_interface():
    ...
    if active_chat:
        ...
        st.markdown(f'<div style="display:flex;align-items:center;gap:8px;padding:8px 0;margin-bottom:8px;border-bottom:1px solid rgba(255,215,0,0.1);">{render_avatar(active_chat, 36)}<div class="username-text">@{html.escape(active_chat)}</div></div>', unsafe_allow_html=True)
        for m in msgs:
            sent = m.get("from") == st.session_state.user
            cls = "sent" if sent else "received"
            align = "flex-end" if sent else "flex-start"
            st.markdown(f'<div style="display:flex;flex-direction:column;align-items:{align};padding:0 4px;"><div class="chat-bubble {cls}">{html.escape(m.get("text", ""))}<div style="font-size:0.6rem;opacity:0.7;text-align:right;margin-top:2px;">{format_timestamp(m["timestamp"])}</div></div></div>', unsafe_allow_html=True)
        with st.form(f"dmf_{active_chat}", clear_on_submit=True):
            c1, c2 = st.columns([5, 1])
            with c1: txt = st.text_input("Message", placeholder="Type a message...", key=f"dmt_{active_chat}")
            with c2:
        ...
        st.markdown(f'<div style="display:flex;align-items:center;gap:8px;padding:8px 0;margin-bottom:8px;border-bottom:1px solid rgba(255,215,0,0.1);"><div class="avatar-placeholder" style="width:36px;height:36px;background:#667eea;">G</div><div><div class="username-text">{html.escape(g.get("name", "Group"))}</div><div style="color:#94a3b8;font-size:0.65rem;">{len(g.get("members", []))} members</div></div></div>', unsafe_allow_html=True)
        for m in msgs:
            sent = m.get("from") == st.session_state.user
            cls = "sent" if sent else "received"
            align = "flex-end" if sent else "flex-start"
            sender = "" if sent else f'<div style="color:#FFD700;font-size:0.65rem;">@{html.escape(m.get("from",""))}</div>'
            st.markdown(f'<div style="display:flex;flex-direction:column;align-items:{align};padding:0 4px;"><div class="chat-bubble {cls}">{sender}{html.escape(m.get("text", ""))}<div style="font-size:0.6rem;opacity:0.7;text-align:right;margin-top:2px;">{format_timestamp(m["timestamp"])}</div></div></div>', unsafe_allow_html=True)
        with st.form(f"grpf_{active_group}", clear_on_submit=True):
            c1, c2 = st.columns([5, 1])
            with c1: txt = st.text_input("Message", placeholder="Type a message...", key=f"grpt_{active_group}")
            with c2:
                if st.form_submit_button("➤"):
        ...
        st.markdown(f'<div style="display:flex;align-items:center;gap:8px;padding:8px 0;margin-bottom:8px;border-bottom:1px solid rgba(255,215,0,0.1);"><div class="avatar-placeholder" style="width:36px;height:36px;background:#f093fb;">C</div><div><div class="username-text">{html.escape(ch.get("name", "Channel"))}</div><div style="color:#94a3b8;font-size:0.65rem;">{len(ch.get("subscribers", []))} subscribers</div></div></div>', unsafe_allow_html=True)
        for m in msgs:
        ...
        if is_admin:
            with st.form(f"chnf_{active_channel}", clear_on_submit=True):
                c1, c2 = st.columns([5, 1])
                with c1: txt = st.text_input("Broadcast", placeholder="Post to channel...", key=f"chnt_{active_channel}")
                with c2:
                    if st.form_submit_button("📢"):
...
def render_create_modal():
    ...
    theme = get_current_theme()
    st.markdown(f'<div class="modal-overlay"><div class="modal-box"><h3 style="color:#FFD700;text-align:center;margin-bottom:12px;">✨ Create Post</h3>', unsafe_allow_html=True)
...
def render_bottom_nav():
    ...
    for i, (tab, icon, label) in enumerate(tabs):
        with cols[i]:
            if current == tab:
                st.markdown(f'<div style="text-align:center;padding:4px;"><div style="font-size:1.3rem;color:#FFD700;">{icon}</div><div style="font-size:0.55rem;color:#FFD700;font-weight:600;">{label}</div></div>', unsafe_allow_html=True)
            else:
...
def render_feed_page():
    ...
    if not posts:
        st.markdown('<div style="text-align:center;padding:3rem 1rem;color:#94a3b8;"><div style="font-size:4rem;">👑</div><h3>Welcome to SocialHub Pro</h3><p>Your premium social experience awaits. Create your first post!</p></div>', unsafe_allow_html=True)
    else:
...
def render_explore_page():
    st.markdown('<div class="content-wrapper">', unsafe_allow_html=True)
    theme = get_current_theme()
    st.markdown(f'<h3 style="color:#FFD700;margin-bottom:8px;">🔍 Explore Users</h3>', unsafe_allow_html=True)
    ...
    for u in filtered[:30]:
        ...
        st.markdown("<hr style='border-color:rgba(255,215,0,0.05);margin:0;'>", unsafe_allow_html=True)
    st.markdown('</div>', unsafe_allow_html=True)
...
def render_chats_page():
    ...
    theme = get_current_theme()
    st.markdown(f'<h3 style="color:#FFD700;margin-bottom:8px;">💬 Messages</h3>', unsafe_allow_html=True)
    ...
    with t1:
        ...
        if chats:
            for c in chats:
                ...
                if st.button("Open", key=f"oc_{c['with_user']}"): st.session_state.active_chat = c['with_user']; st.rerun()
    ...
    with t2:
        ...
        if groups:
            for g in groups:
                ...
                if st.button("Open", key=f"og_{g['id']}"): st.session_state.active_group = g['id']; st.rerun()
...
def render_profile_page():
    ...
    st.markdown(f'<div style="text-align:center;padding:16px 0;">{render_avatar(user, 72)}<h2 style="color:#FFD700;margin-top:8px;">@{html.escape(user)}</h2><p style="color:{theme["secondary"]};font-size:0.85rem;">{html.escape(profile.get("bio", "No bio yet"))}</p></div>', unsafe_allow_html=True)
    st.markdown(f'<div style="display:flex;justify-content:space-around;text-align:center;padding:12px;border-top:1px solid rgba(255,215,0,0.1);border-bottom:1px solid rgba(255,215,0,0.1);margin-bottom:12px;"><div><div style="color:#FFD700;font-size:1.2rem;font-weight:700;">{profile.get("post_count", 0)}</div><div style="color:{theme["secondary"]};font-size:0.6rem;">Posts</div></div><div><div style="color:#FFD700;font-size:1.2rem;font-weight:700;">{len(profile.get("followers", []))}</div><div style="color:{theme["secondary"]};font-size:0.6rem;">Followers</div></div><div><div style="color:#FFD700;font-size:1.2rem;font-weight:700;">{len(profile.get("following", []))}</div><div style="color:{theme["secondary"]};font-size:0.6rem;">Following</div></div></div>', unsafe_allow_html=True)
    ...
    with st.expander("✏️ Edit Profile"):
        with st.form("epf"):
            ...
            if st.form_submit_button("💾 Save", use_container_width=True):
                updates = {"bio": sanitize_text(bio, MAX_BIO_LENGTH), "website": sanitize_text(website, 100), "location": sanitize_text(location, 100), "gender": gender}
    ...
    with st.expander("🎨 Themes (20+)"):
        ...
        for tkey, tdata in THEMES.items():
            ...
            if st.button("Apply", key=f"th_{tkey}"): DataManager.update_profile(user, {"theme": tkey}); st.rerun()
        st.markdown('</div>', unsafe_allow_html=True)
    ...
    with st.expander("🖼️ Wallpapers (30)"):
        ...
        for wp_key, wp_data in WALLPAPERS.items():
            ...
            if st.button("Apply", key=f"wp_{wp_key}"): DataManager.update_profile(user, {"wallpaper": wp_key}); st.rerun()
        st.markdown('</div>', unsafe_allow_html=True)
    
    posts = [p for p in st.session_state.feed_posts if p.get("username") == user]
    if posts:
        st.markdown(f'<h4 style="color:#FFD700;margin-top:12px;">Your Posts</h4>', unsafe_allow_html=True)
